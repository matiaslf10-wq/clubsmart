...
export default async function PaymentBatchesPage({
  ...
  const parsedYear =
    Number(parameters.anio);

  const parsedMonth =
    Number(parameters.mes);
  ...
  if (
    ...
    if (error) {
      throw new Error(
        `No fue posible cargar las adhesiones: ${error.message}`,
      );
    }
      ...
      <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="mt-3 max-w-3xl text-slate-600">
            Agrupá cuotas pendientes,
            verificá adhesiones y prepará
            el cobro antes de enviarlo al
            proveedor.
          </p>
        </div>
      ...
      {!configurationReady ? (
        <div className="mt-6 rounded-xl border border-amber-200 bg-amber-50 p-5 text-amber-900">
          <p className="font-semibold">
            Pago TIC todavía no está
            conectado
          </p>

          <p className="mt-2 text-sm leading-6">
            Podés crear y revisar lotes,
            pero sus cuotas no quedarán
            habilitadas para envío hasta
      ...
      <section className="mt-6 rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="border-b border-slate-200 p-6">
          ...
          <p className="mt-2 text-sm leading-6 text-slate-600">
            Las cuotas sin adhesión o con
            problemas también pueden
            incluirse para identificarlas
            como bloqueadas durante la
            revisión.
          </p>
        </div>
